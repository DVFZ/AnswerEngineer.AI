...
export class GEOAudit {
    ...
    static calculateWeightedScore(points) {
    ...
    static generateRecommendations(points) {
        const recommendations = [];
        points.forEach((point) => {
            // Extract critical issues (fails)
            const failures = point.findings.filter((f) => f.startsWith('✗'));
            failures.forEach((f) => {
                recommendations.push(`[${point.name}] ${f}`);
            });
            // Extract warnings
            const warnings = point.findings.filter((f) => f.startsWith('⚠'));
...
//# sourceMappingURL=geo-audit.js.map