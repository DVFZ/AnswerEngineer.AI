...
export class CTRSignalsAuditor {
  ...
  private static findCTAElements(): { count: number; types: string[] } {
    const types: string[] = [];
    let count = 0;
  ...
  private static analyzeHeadlines(): {
    h1Count: number;
    h1Text: string;
  ...
  private static analyzeReadability(): {
  ...
  private static analyzeFormatting(): {
    lists: number;
    bolds: number;
  } {
  ...
  private static checkImageAltText(): {
    total: number;
    withAlt: number;
    withoutAlt: number;
  } {
