...
export class JsHandlingAuditor {
  static audit(pageData: PageData): AuditPoint {
  ...
  private static hasCriticalContentInHTML(): boolean {
    ...
    return (document.body as HTMLElement).innerText.length > 500;
  }

  private static estimateScriptErrors(): number {
  ...
  private static checkRenderBlockingScripts(): { count: number; urls: string[] } {
    const blocking = { count: 0, urls: [] as string[] };
  ...
  private static checkDeferredAsyncScripts(): { async: number; defer: number } {
    return {
  ...
  private static checkThirdPartyScripts(): number {
