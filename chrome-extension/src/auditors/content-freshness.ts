...
export class ContentFreshnessAuditor {
  ...
  private static extractPublicationDate(): Date | null {
    // Check meta tags
    const tags = [
  ...
  private static extractModifiedDate(): Date | null {
    const tags = [
  ...
  private static getDaysOld(date: Date): number {
    const now = new Date();
    const diff = now.getTime() - date.getTime();
    return Math.floor(diff / (1000 * 60 * 60 * 24));
  }

  private static getUpdateFrequency(): string | null {
  ...
  private static findRecentDateReferences(): number {
  ...
  private static hasChangelogOrHistory(): boolean {
